...
export default function AdminSidebar({ isOpen = false, onClose = () => { } }) {
  return (
    <>
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(item => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 radius-2xl text-sm font-semibold transition ${isActive
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-ghibli-forest text-ghibli-cream flex flex-col overflow-y-auto transform transition-transform duration-200 sm:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        ...
        <nav className="p-4 space-y-1">
          {navItems.map(item => (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `block px-4 py-3 radius-2xl text-sm font-semibold transition ${isActive
                  ? 'bg-ghibli-cream text-ghibli-forest shadow-soft-sm'
                  : 'text-ghibli-cream/70 hover:bg-ghibli-cream/10 hover:text-ghibli-cream'
                }`
              }
